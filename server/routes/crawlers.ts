  ...
  const demoCrawlers: Crawler[] = [
    {
      ...
      description:
        "Monitors product prices across multiple e-commerce sites to track competitor pricing and identify market trends.",
    ...
    },
    {
      ...
      description:
        "Collects and categorizes technology news articles from various sources for content analysis and trend identification.",
    ...
    },
    {
      ...
      description:
        "Extracts and analyzes social media posts to gauge public sentiment about specific brands and products.",
    ...
    },
    {
      ...
      description:
        "Gathers job postings from various job boards to analyze market demand and salary trends in different industries.",
    ...
    },
    {
      ...
      description:
        "Scrapes property listings to track real estate market trends, pricing patterns, and availability in target areas.",
    ...
    },
    {
      ...
      description:
        "Collects historical weather data for climate analysis and predictive modeling across multiple geographic regions.",
      status: CrawlerStatus.COMPLETED,
      created_by: "demo-user-123",
...
export const getCrawlers: RequestHandler = async (req, res) => {
  try {
    ...
    let userCrawlers = crawlers.filter(
      (crawler) => crawler.created_by === userId,
    );

    // Apply status filter
    if (status && Object.values(CrawlerStatus).includes(status)) {
      userCrawlers = userCrawlers.filter(
        (crawler) => crawler.status === status,
      );
    }
...
export const getCrawler: RequestHandler = async (req, res) => {
  try {
    ...
    const { id } = req.params;
    const crawler = crawlers.find(
      (c) => c.id === id && c.created_by === userId,
    );
...
export const updateCrawler: RequestHandler = async (req, res) => {
  try {
    ...
    const crawlerIndex = crawlers.findIndex(
      (c) => c.id === id && c.created_by === userId,
    );
    if (crawlerIndex === -1) {
      return res.status(404).json({ message: "Crawler not found" });
...
export const getCrawlerStats: RequestHandler = async (req, res) => {
  try {
    ...
    const userCrawlers = crawlers.filter(
      (crawler) => crawler.created_by === userId,
    );
    ...
    const stats = {
      ...
      by_status: {
        [CrawlerStatus.TODO]: userCrawlers.filter(
          (c) => c.status === CrawlerStatus.TODO,
        ).length,
        [CrawlerStatus.IN_PROGRESS]: userCrawlers.filter(
          (c) => c.status === CrawlerStatus.IN_PROGRESS,
        ).length,
        [CrawlerStatus.READY_FOR_QA]: userCrawlers.filter(
          (c) => c.status === CrawlerStatus.READY_FOR_QA,
        ).length,
        [CrawlerStatus.COMPLETED]: userCrawlers.filter(
          (c) => c.status === CrawlerStatus.COMPLETED,
        ).length,
        [CrawlerStatus.FAILED]: userCrawlers.filter(
          (c) => c.status === CrawlerStatus.FAILED,
        ).length,
      },
    };
