...
export const operationNotify = {
  ...
  saveError: (itemType?: string) => {
    quickNotify.error(
      `Failed to save ${itemType || "changes"}. Please try again.`,
    );
  },

  loadError: (itemType?: string) => {
    quickNotify.error(
      `Failed to load ${itemType || "data"}. Please refresh the page.`,
    );
  },
  ...
  permissionWarning: () => {
    quickNotify.warning(
      "Limited permissions. Some features may not be available.",
    );
  },
};
...
export const systemNotify = {
  ...
  dataExported: (format?: string) => {
    quickNotify.success(
      `Data exported successfully${format ? ` as ${format}` : ""}.`,
    );
  },
...
export const batchNotify = {
  ...
  processItems: async <T, R>(
    ...
    const progress = options.showProgress
      ? new ProgressNotification(options.title)
      : null;
    const results: R[] = [];
    const errors: Error[] = [];
    ...
    if (progress) {
      if (errors.length === 0) {
        progress.complete(
          options.successMessage?.(results) ||
