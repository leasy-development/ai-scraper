...
import { useApiNotifications, useCrawlerNotifications, usePropertyNotifications } from '@/hooks/useApiNotifications';
import { DashboardLayout } from '@/components/dashboard/DashboardLayout';

export default function NotificationDemo() {
  const { notifications, unreadCount } = useNotifications();
  const { success, error, warning, info, loading, dismiss } = useNotificationActions();
  ...
  // Demo API operations
  const simulateApiOperation = async (type: 'success' | 'error', delay = 2000) => {
  ...
  const runProgressDemo = () => {
    const progress = new ProgressNotification('Processing Data', 'Initializing...');
    ...
    const interval = setInterval(() => {
      ...
      if (currentProgress <= 100) {
        progress.updateProgress(currentProgress, `Processing step ${currentProgress / 20}...`);
      }
  ...
  const runBatchDemo = async () => {
    ...
    await batchNotify.processItems(
      ...
      },
      {
        ...
        showProgress: true,
        successMessage: (results) => `Successfully processed ${results.length} items`,
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              ...
              <Button 
                size="sm"
                onClick={() => operationNotify.copied('URL')}
              >
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <Button onClick={runProgressDemo}>
                Progress Demo
              </Button>
              <Button onClick={runBatchDemo}>
                Batch Processing
              </Button>
            </div>
          ...
          {showInlineDemo && (
            <CardContent className="space-y-4">
              ...
              <WarningNotification
                ...
                action={{
                  label: 'Upgrade Now',
                  onClick: () => info('Upgrade', 'Redirecting to upgrade page...'),
                }}
              />
