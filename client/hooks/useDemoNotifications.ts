    ...
    if (isAuthenticated && user?.email === "demo@aiscraper.com") {
      ...
      if (!hasGenerated) {
        ...
        setTimeout(() => {
          success(
            "Crawler Started",
            "E-commerce product scraper is now running successfully",
          );
          info(
            "System Update",
            "Dashboard has been updated with new notification features",
          );
          warning(
            "Rate Limit Warning",
            "API request rate is approaching limits for amazon.com crawler",
          );
          error(
            "Connection Error",
            "Failed to connect to news.ycombinator.com - retrying in 30 seconds",
          );
