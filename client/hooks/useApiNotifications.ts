...
export function useApiNotifications() {
  ...
  const batch = useCallback(async <T>(
    ...
    const loadingId = loading(`Processing ${operations.length} ${itemType}(s)...`);
    const results: T[] = [];
    const errors: Error[] = [];
    ...
    try {
      ...
      if (errors.length === 0) {
        if (options?.showSuccessNotification !== false) {
          success(`All ${itemType}(s) processed successfully`, `${results.length} items completed`);
        }
        ...
        error(`Processing failed`, `All ${errors.length} ${itemType}(s) failed to process`);
      }

      return results;
    } catch (err) {
      dismiss(loadingId);
      error('Batch operation failed', 'An unexpected error occurred during batch processing');
      throw err;
    }
